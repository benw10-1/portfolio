...
const baseWords = ["love", "JavaScript", "developer", "Python", "documentation", "API", "modular code", "Node", "js", "summary", "passionate", "Django", "Neo4J", "dynamically", "Hackerrank", "LeetCode", "problem solving", "critical thinking", "OpenLayers"]
...
function loadSocials() {
    ...
    for (const x of socialS) {
        ...
        if (x[1].indexOf("://") >= 0) {
            ...
            img.addEventListener("click", event => {
                navigator.clipboard.writeText(x[1])
...
function loadEls() {
    ...
    logo.addEventListener("click", function (event) {
        if (scrolling || selected === 0) return
        scrolling = true
        aniLogo()
        loadArrows()
        window.scroll({
...
function getWork() {
    return fetch(("http://" + window.location.host + "/api/getProjects").trim()).then((data) => {return data.json()}).then((data) => {
        for (const x of data) {
            let work = loadWork(x)
...
function displayModal(state) {
    if (state) modal1.classList.remove("hidden")
    else modal1.classList.add("hidden")
    modalled = state
}
...
function setTip(tip, fadeTime=1000) {
    ...
    if (mouseTm) clearTimeout(mouseTm)
    else {
        mouseTip.style.left = "0"
        mouseTip.style.top = "-10%"
